 ...
 *     a PIN, but the card must be programmed; use @c cryptnox @c init).
 *
 ...
 * speaking. Halts on init failure so the user can inspect the Serial
 * output.
 */
void setup() {
 ...
 *   - @ref CryptnoxWallet::getCardInfo fetches card identity over the
 *     channel; on enabled debug logging it prints the card info bytes.
